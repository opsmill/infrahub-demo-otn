...
![The fourteen PoPs of the modelled European optical core, with the 21 multiplex sections coloured by the largest ODU that still fits on the roomiest lit carrier. Vienna to Milan is red at 240 of 240 slots committed, Paris to Frankfurt amber at 953 of 960, Berlin to Frankfurt blue at 1552 of 1600, Amsterdam to Frankfurt and Frankfurt to Milan green, and the remaining sixteen routes grey and dashed.](./media/odu-map.svg)
  ...
  branch shows only two of them: `odu4` on eight sections and `no-odu` on
  thirteen. The measured split in this picture is `odu4` 2, `odu2` 1, `odu0` 1,
  `full` 1 and `no-odu` 16, three sections greyer than a copy taken today: it was
  rendered before the three router-to-router OpenZR+ wavelengths joined the
  dataset. Every figure in the panel was checked against the containers on the
...
On a clean `main` the grey band covers **13 of the 21 sections**, and most of the
map is grey. That is the honest picture and not a broken render.

The 13 are sections with **no wavelength at all**, not sections whose wavelengths
...
`objects/17_geant_carriers.yml` cover eight sections and nothing else:
