...
```text
16 multiplexer(s) examined, 90 client port(s) judged for binding exactly one
...
cannot say "any of these three kinds and none of the other five". Declaring the
reverse edge on the port instead would put a transceiver field on amplifier and
ROADM degree ports that never hold one.
...
and holding one is why the port relationship is optional. That optionality is
also why the duplicate rule is here rather than in the schema
```

**The other half of this check has no branch, and the reason is measured.** A
port holding two optics is what `uniqueness_constraints: [["port"]]` exists for,
and Infrahub accepts that constraint only where the relationship is mandatory:
with `optional: true` the schema load is refused outright. Making the port
mandatory buys the constraint and costs the ability to hold an unfitted optic,
which is three of the nine units here and most of what an optic inventory is
for. The constraint is available and it is the wrong trade, so the duplicate
rule sits in the check beside the port kind.
...
**Negative result: polish is judged on pumped glass and nowhere else.** The 124
unpumped spans terminate on APC degree ports and the check says nothing about
them, and it says nothing about the 126 line ports either, because none of them
